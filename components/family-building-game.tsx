...
function ConfettiBurst({ active }: { active: boolean }) {
  ...
  return (
    <div
      className="family-game__confetti pointer-events-none absolute inset-0 overflow-hidden"
      aria-hidden="true"
    >
      {Array.from({ length: 14 }, (_, i) => (
        <span
          ...
          style={{
            ...
            backgroundColor:
              i % 3 === 0 ? "#9B3A2F" : i % 3 === 1 ? "#1C1C1C" : "#C4A574",
            width: i % 2 === 0 ? 6 : 4,
            height: i % 2 === 0 ? 6 : 4,
          }}
        />
...
function SituationQuizPanel({
  ...
  return (
    <div className="family-game__quiz mt-6 rounded-sm border-2 border-[#9B3A2F]/20 bg-white/90 p-5">
      <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#9B3A2F]">
        {quiz.context}
      </p>
      <h4 className="mt-2 font-serif text-lg leading-snug text-[#1C1C1C]">
        {quiz.question}
      </h4>
          ...
          return (
            <button
              ...
              className={cn(
                ...
                !answered &&
                  "cursor-pointer hover:border-[#9B3A2F]/30 hover:bg-[#9B3A2F]/5",
                !answered && "border-[#1C1C1C]/12 bg-[#F9F6F0]/50",
                showResult &&
                  opt.correct &&
                  "border-[#9B3A2F]/40 bg-[#9B3A2F]/10",
                showResult &&
                  !opt.correct &&
                  "border-[#9B3A2F]/30 bg-[#9B3A2F]/8",
...
function FamilyEventPanel({
  ...
  return (
    <div className="family-game__quiz mt-6 rounded-sm border-2 border-[#1C1C1C]/15 bg-white/90 p-5">
      ...
      <p className="mt-2 text-sm leading-relaxed text-[#1C1C1C]/80">
        {event.description}
      </p>
      <div className="mt-4 space-y-2">
        {options.map((opt) => {
...
function IntroPanel({
  ...
  return (
    <div className="family-game__intro mx-auto max-w-2xl rounded-sm border border-[#1C1C1C]/10 bg-white/70 p-8 text-center shadow-[0_16px_48px_rgba(28,28,28,0.05)] md:p-10">
      ...
      <p className="font-serif text-lg italic leading-relaxed text-[#1C1C1C]/80">
        {INTRO_MESSAGE.headline}
      </p>
      <p className="mt-3 text-sm leading-relaxed text-[#1C1C1C]/70 md:text-base">
        {INTRO_MESSAGE.body}
      </p>
      ...
      <ul className="mt-6 space-y-2 text-left text-sm text-[#1C1C1C]/65">
        <li className="flex gap-2">
          <Zap
            className="mt-0.5 size-4 shrink-0 text-[#9B3A2F]"
            aria-hidden="true"
          />
...
export function FamilyBuildingGame() {
  ...
  useLayoutEffect(() => {
    ...
    const el = section.querySelector(
      '[aria-pressed="true"][aria-label*="' + arrivingId + '"]',
    ) as HTMLElement | null;
    const ctx = gsap.context(() => {
      if (el) {
        gsap.fromTo(
  ...
  const availableCards = useMemo(
    () =>
      roundCards.filter(
        (card) =>
          !usedCardIds.includes(card.id) && !hintedOutIds.includes(card.id),
      ),
  ...
  const handleHint = useCallback(() => {
    ...
    setFeedback({
      type: "tip",
      message: `Gợi ý: Loại bỏ "${pick.label}"`,
  ...
  const handleCardClick = useCallback(
    (card: GameCard) => {
      ...
      if (card.isCorrect) {
        ...
        const partNote = card.housePart
          ? HOUSE_PARTS[card.housePart].academicNote
          : card.description;
        if (combo) {
          setFeedback({
            type: "combo",
      ...
      setFeedback({
        ...
        description: [
          ...
          opposites.length
            ? `Giá trị nên chọn: ${opposites.join(", ")}`
            : undefined,
        ]
          .filter(Boolean)
          .join(" "),
  ...
  const handleEventAnswer = useCallback(
    (correct: boolean, explanation: string) => {
      ...
      setFeedback({
        ...
        message: correct
          ? "Ứng xử phù hợp!"
          : "Chưa phù hợp trong tình huống này.",
        description: explanation,
      });
      if (correct) triggerConfetti();
    },
      ...
      <div className="relative mx-auto max-w-6xl">
        <div className="mb-14 flex flex-col items-center text-center">
          ...
          <p className="mt-5 max-w-2xl text-base leading-relaxed text-[#1C1C1C]/75">
            Chọn giá trị đúng để xây nền móng, cột trụ và mái nhà — ôn tập kiến
            thức chương 7.
          </p>
        </div>
          ...
          <>
              ...
              <div className="flex flex-wrap items-center gap-2">
                {presentationMode ? (
                  ...
                  <span className="inline-flex items-center gap-1.5 rounded-sm border border-[#1C1C1C]/10 bg-white/60 px-3 py-1.5 font-mono text-xs text-[#1C1C1C]/70">
                    <Clock
                      className="size-3.5 text-[#9B3A2F]"
                      aria-hidden="true"
                    />
                    {formatTime(elapsed)}
                  </span>
                ...
                <span className="inline-flex items-center gap-1.5 rounded-sm border border-[#1C1C1C]/10 bg-white/60 px-3 py-1.5 font-mono text-xs text-[#1C1C1C]/70">
                  Văn minh:{" "}
                  <span className="text-[#9B3A2F]">{civilityLive}%</span>
                </span>
                {streak >= 2 && (
            ...
            <div className="grid grid-cols-1 gap-10 lg:grid-cols-[1fr_360px] lg:gap-12">
              <div className="order-2 lg:order-1">
                <div className="mb-4 flex items-center justify-between gap-4">
                  <p className="font-mono text-xs uppercase tracking-[0.2em] text-[#1C1C1C]/55">
                    Chọn giá trị
                  </p>
                  <p className="font-mono text-xs text-[#1C1C1C]/45">
                    {availableCards.length} thẻ
                  </p>
                </div>

                <div
                  className="family-game__deck grid grid-cols-1 gap-3 sm:grid-cols-2"
                  role="group"
                  aria-label="Danh sách thẻ giá trị"
                >
                  {roundCards.map((card) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          isPlaced &&
                            "family-game__card--placed border-[#9B3A2F]/35 bg-[#9B3A2F]/8",
                          ...
                          isUsed &&
                            !isPlaced &&
                            "family-game__card--wrong opacity-60",
                          isHintedOut &&
                            "family-game__card--hinted line-through opacity-40",
                          (isUsed || isHintedOut || isBlocked) &&
                            "cursor-default",
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center justify-between gap-2">
                              ...
                              {isPlaced && (
                                <Sparkles
                                  className="size-3 shrink-0 text-[#9B3A2F]/70"
                                  aria-hidden="true"
                                />
                              )}
                            </div>
                            <p className="mt-1.5 text-xs leading-relaxed text-[#1C1C1C]/55">
                              {card.example}
                            </p>
                    ...
                    {eventAnswered && (
                      <div className="mt-3 text-center">
                        <Button
                          ...
                          className="rounded-sm text-xs uppercase tracking-[0.15em]"
                        >
                          Tiếp tục
                        </Button>
                      </div>
                  ...
                  <div
                    ...
                    className={cn(
                      ...
                      feedback.type === "correct" &&
                        "border-[#9B3A2F]/20 bg-[#9B3A2F]/5",
                      feedback.type === "combo" &&
                        "family-game__feedback--combo border-[#9B3A2F]/30 bg-[#9B3A2F]/10",
                      feedback.type === "wrong" &&
                        "border-[#9B3A2F]/25 bg-[#9B3A2F]/8",
                      feedback.type === "tip" &&
                        "border-[#1C1C1C]/12 bg-white/80",
                      feedback.type === "info" &&
                        "border-[#1C1C1C]/12 bg-white/80",
                      feedback.type === "event" &&
                        "border-[#1C1C1C]/12 bg-white/80",
                ...
                {phase === "playing" &&
                  ...
                  !isBlocked && (
                    <div className="mt-4 rounded-sm border border-dashed border-[#9B3A2F]/20 bg-[#9B3A2F]/5 px-4 py-3">
                      ...
                      <p className="mt-2 text-sm leading-relaxed text-[#1C1C1C]/75">
                        {ACADEMIC_TIPS[tipIndex]}
                      </p>
                    </div>
                  )}
              </div>
                  ...
                  <div className="mt-6 space-y-3">
                    ...
                    <div className="h-1.5 overflow-hidden rounded-full bg-[#1C1C1C]/8">
                      <div
                        ...
                        style={{
                          width: `${(progress / TOTAL_CORRECT) * 100}%`,
                        }}
                      />
                    </div>
                    <p className="font-mono text-xs text-[#1C1C1C]/50">
                      Điểm:{" "}
                      <span className="text-[#9B3A2F]">
                        {phase === "finished" ? finalScore : score}
                      </span>{" "}
                      / {TOTAL_CORRECT}
                    </p>
                  </div>
            ...
            <div className="mt-10 flex justify-center">
              {phase === "playing" ? (
                ...
                <Button
                  ...
                  className="h-10 min-w-[200px] rounded-sm text-xs uppercase tracking-[0.2em]"
                >
            ...
            {phase === "finished" && finalScore != null && (
              <div className="mx-auto mt-10 max-w-2xl space-y-6">
                <div
                  className="family-game__result rounded-sm border border-[#9B3A2F]/20 bg-white/80 p-8 shadow-[0_16px_48px_rgba(28,28,28,0.06)]"
                  role="dialog"
                >
                  <div className="text-center">
                    ...
                    <h3 className="mt-4 font-serif text-3xl text-[#1C1C1C]">
                      {finalScore}/{TOTAL_CORRECT} điểm
                    </h3>
                    <p className="mt-2 font-mono text-xs text-[#1C1C1C]/50">
                      Văn minh gia đình: {civility}% · Đúng{" "}
                      {placedCorrect.length} · Sai {wrongAttempts}
                      {!presentationMode && ` · ${formatTime(elapsed)}`}
                    </p>
                    <p className="mt-4 text-base leading-relaxed text-[#1C1C1C]/80">
                    ...
                    <div className="mt-6 rounded-sm border border-[#1C1C1C]/10 bg-[#F9F6F0]/60 p-4">
                      ...
                      <ul className="mt-3 space-y-3">
                        {missedCards.map((card) => (
                          <li
                            key={card.id}
                            className="text-sm text-[#1C1C1C]/75"
                          >
                            <strong className="font-serif text-[#9B3A2F]">
                              Thiếu {card.label}
                            </strong>
                            <p className="mt-1 text-xs leading-relaxed text-[#1C1C1C]/65">
                              {card.missingConsequence}
                            </p>
                          </li>
                        ))}
                      </ul>
                ...
                <div className="rounded-sm border border-[#1C1C1C]/10 bg-white/70 p-6">
                  ...
                  <div className="mt-4 flex flex-col gap-2 sm:flex-row">
                    ...
                    <Button
                      ...
                      className="rounded-sm bg-[#9B3A2F] text-xs uppercase tracking-[0.15em] text-white hover:bg-[#9B3A2F]/90"
                    >
                      Lưu điểm
                    </Button>
                  </div>
                      ...
                      {leaderboard.map((entry, i) => (
                        ...
                        >
                          ...
                          <span className="font-mono text-xs text-[#1C1C1C]/55">
                            {entry.score}/{TOTAL_CORRECT} · {entry.civility}% ·{" "}
                            {entry.date}
                          </span>
                        </li>
                      ))}
